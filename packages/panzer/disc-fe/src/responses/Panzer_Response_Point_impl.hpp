#ifndef __Panzer_Response_Point_impl_hpp__
#define __Panzer_Response_Point_impl_hpp__
...
Response_Point<EvalT>::
Response_Point(const std::string & responseName, MPI_Comm comm,
  ...
  : ResponseMESupport_Default<EvalT>(responseName,comm), value(0.0),
    have_point(false), linObjFactory_(linObjFact)
{
...
scatterResponse()
{
  ...
  // find the minimum processor who has the point value
  int locProc = have_point ? this->getComm()->getRank() : this->getComm()->getSize();
  int glbProc = 0;
...
template < >
void Response_Point<panzer::Traits::Jacobian>::
scatterResponse()
{
...
template < >
void Response_Point<panzer::Traits::Hessian>::
scatterResponse()
{
  using Teuchos::rcp_dynamic_cast;
...
template < >
void Response_Point<panzer::Traits::Tangent>::
scatterResponse()
{
