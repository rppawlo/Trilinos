...
struct SacadoViewFill
{
  ...
  {
    Kokkos::printf("ROGER Starting SacadoViewFill!");
  ...
  SacadoViewFill( const OutputView & arg_out , const_value_type & arg_in )
    : output( arg_out ), input( arg_in )
    {
      ...
      Kokkos::parallel_for( "SacadoViewFill for constant scalar",policy, *this );
    }
};
  ...
  // ROGER HACK COPY
template <class OutputView, typename Enabled = void>
struct SacadoViewFill_ROGER_HACK
{
  typedef typename OutputView::const_value_type  const_value_type ;
  typedef typename OutputView::execution_space execution_space ;
  ...
  SacadoViewFill_ROGER_HACK( const OutputView & arg_out , const_value_type & arg_in )
    : output( arg_out ), input( arg_in )
    {
      const size_t n0 = output.extent(0);
      Kokkos::RangePolicy<execution_space> policy( 0, n0 );
  ...
  )>::type * = 0 )
{
  ...
  Impl::SacadoViewFill_ROGER_HACK< View<DT,DP...> >( view , value );
}
